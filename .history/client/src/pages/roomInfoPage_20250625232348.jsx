...
export default function RoomInfoPage() {
  ...
  const nights = localDates && localDates[0]?.startDate && localDates[0]?.endDate ? Math.max(
        1,
        Math.ceil(
          (new Date(localDates[0].endDate).setHours(0,0,0,0) -
           new Date(localDates[0].startDate).setHours(0,0,0,0)) /
            (1000 * 60 * 60 * 24)
        ) || 1
      ) : 1;
...
7const totalPrice = totalPricePerNight * nights;
