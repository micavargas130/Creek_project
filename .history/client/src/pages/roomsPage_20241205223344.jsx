...
export default function RoomsPage() {
 ...
 const dates = location.state?.dates || contextDates;
  const options = location.state?.options || contextOptions;

  useEffect(() => {
    axios.get('/lodges/').then(response => {
      setLodges(response.data);
    });
  }, [dates, options]); // Use effect when these change
}
          ...
          {filteredLodges.length === 0 ? (
            <div className="noRooms">
              ...
              <div className="activity-images">
                ...
                <img src="src/assets/asador.jpeg" className="activity-image" alt="Asador" />
