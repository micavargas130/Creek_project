...
export default function RoomsPage() {
  ...
  useEffect(() => {
    ...
    axios.get('/lodges').then(response => {
      const filteredLodges = response.data.filter(item =>
        item.capacity >= adult + children && isAvailable(item)
      );
      setLodges(filteredLodges);
    });
  }, [dates, options]); // Dependencia en fechas y opciones
  ...
  const handleClick = () => {
    // Lógica de búsqueda dentro del mismo componente, sin redirigir
    axios.get('/lodges').then(response => {
      const filteredLodges = response.data.filter(item =>
        item.capacity >= adult + children && isAvailable(item)
      );
