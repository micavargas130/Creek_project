...
export default function RoomInfoPage() {
  ...
  const [price, setPrice] = useState({ priceAdult: 0, priceChildren: 0 }); // Inicializa con valores por defecto
  ...
  useEffect(() => {
    const fetchPrice = async () => {
      try {
        ...
        setPrice(response.data); // Asume que response.data tiene las propiedades priceAdult y priceChildren
      } catch (error) {
  ...
  const totalPrice = ((options.adult * price.priceAdult) + (options.children * price.priceChild));
  ...
  return (
    ...
    <div className="lodgeImgWrapper" key={i}>
      <img
        src={photo ? `http://localhost:3000/${photo}` : "ruta/a/imagen_por_defecto.jpg"}
        alt={data.name}
        className="lodgeImg"
      />
    </div>
        ...
        <div className="lodgeWrapper">
          ...
          <div className="lodgeImages">
            {data.photos?.map((photo, i) => (
              <div className="lodgeImgWrapper" key={i}>
                <img
               src={data.photos[0] ? `http://localhost:3000/${data.photos[0]}` : "ruta/a/imagen_por_defecto.jpg"}
               alt={data.name}
               className="siImg"
            />
              </div>
  ...
  <div className="peopleSetter">
    ...
    {openOptions && (
      <div className="optionsPeople">
              ...
              <b>Price: {totalPrice} per night</b>
