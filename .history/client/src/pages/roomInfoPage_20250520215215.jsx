...
export default function RoomInfoPage() {
  ...
  useEffect(() => {
    ...
    const fetchPrice = async () => {
      try {
        const response = await axios.get("http://localhost:3000/prices?category=cabanas");
    ...
    const fetchOccupiedDates = async () => {
      try {
        const response = await axios.get(`http://localhost:3000/lodges/occupied-dates/${id}`);
...
const totalStayPrice = totalPricePerNight * nights;
         ...
         <div className="lodgeDetailsPrice">
           ...
           <div className="peopleSetter">
             ...
             {openOptions && (
               ...
               <div>
                <h2>Detalle:</h2>
                <p>Precio por adulto: ${price.priceAdult}</p>
                <p>Precio por niño: ${price.priceChild}</p>
                <p>Cantidad de noches: {nights}</p>
                <p>Cantidad de adultos: {options.adult}</p>
                <p>Cantidad de niños: {options.children}</p>
                <p>Subtotal por noche: ${totalPricePerNight}</p>
                <h3>Total a pagar: ${totalStayPrice}</h3>
              </div>
