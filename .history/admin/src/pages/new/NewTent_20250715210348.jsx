...
const New = () => {
  ...
  const [selectedStatus, setSelectedStatus] = useState("pagado");
  ...
  const handleDateChange = (checkOut) => {
    ...
    let daysDifference = Math.round((checkOutDate - checkInDate) / (1000 * 3600 * 24)); //diferencia de dias entre fecha check-in y check-out
  ...
  const newTent = async (ev) => {
    ...
    // Abrir el modal para seleccionar el estado delpago
  ...
  const confirmFinishClick = async () => {
    ...
    try {
        ...
        const totalAmount = tentResponse.data.totalAmount; 
            ...
            <div className="mb-64">
                  <div>
                    <div>
                      ...
                      DNI
                      <input type="number" id="dni" placeholder="Ingrese su DNI" value={dni || ""} onChange={(e) => {
                      ...
                      Telefono
                      <input type="text" placeholder="Telefono" value={phone} onChange={(ev) => setPhone(ev.target.value)} />
                      Ocupación
                      <input type={"text"} placeholder="Ocupación" value={ocupation} onChange={(ev) => setOcupation(ev.target.value)} />
                    </div>
                    <div>
                      Check-In
                      <input type="date" placeholder="check-in" value={checkIn} onChange={(ev) => setCheckIn(ev.target.value)} disabled/>
                      Check-Out
                      <input type={"date"} placeholder="check-out" value={checkOut} onChange={(ev) => setCheckOut(ev.target.value)} />
                      Cantidad de adultos
                      <input type="number" placeholder="Cantidad de adultos" value={numberOfAdults}  min={0}  onChange={(ev) => {
                      ...
                      />
                      Cantidad de niños
                      <input type="number" placeholder="Cantidad de niños" value={numberOfChildren} min={0} onChange={(ev) => {
