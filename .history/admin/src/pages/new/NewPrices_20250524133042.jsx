...
const New = () => {
  ...
  useEffect(() => {
    const fetchPricesData = async () => {
      try {
        ...
        const lastCabañasPrice = cabañasPrices[cabañasPrices.length - 1] || {};
        const lastCarpasPrice = carpasPrices[carpasPrices.length - 1] || {};

        setFormData({
          cabañas: lastCabañasPrice,
          carpas: lastCarpasPrice,
        });

        setPriceHistory(pricesRes.data.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)));
      } catch (err) {
  ...
  const handleFormSubmit = async () => {
    try {
      ...
      await Promise.all([
        axiosInstance.post("/prices", { priceAdult: formData.cabañas.priceAdult, priceChild: formData.cabañas.priceChild, category: "cabañas" }),
                  ...
                  <p className="lodgeDesc">
                    {editing ? (
                      <input
                        type="number"
                        name="priceAdult"
                        value={formData.cabañas.priceAdult || ""}
                        onChange={(e) => handleInputChange(e, "cabañas")}
                        className="editInput"
                      />
                    ) : (
                      formData.cabañas.priceAdult
                    )}
                  </p>
                  ...
                  <p className="lodgeCapacity">
                    {editing ? (
                      <input
                        type="number"
                        name="priceChild"
                        value={formData.cabañas.priceChild || ""}
                        onChange={(e) => handleInputChange(e, "cabañas")}
                        className="editInput"
                      />
                    ) : (
                      formData.cabañas.priceChild
                    )}
                  </p>
