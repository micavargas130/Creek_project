...
const New = () => {
  ...
  const [partialPayment, setPayment] = useState([]); //Lo que paga la persona
  ...
  const confirmFinishClick = async () => {
    ...
    try {
      ...
      const calculatedTotalAmount = (numberOfAdults * price.priceAdult) + (numberOfChildren * price.priceChild);
     ...
     if (selectedStatus === "pagada") {
       ...
       payment = Payment; // Pago parcial ingresado por el usuario
       }
      const paymentData = {
        amount: payment,
