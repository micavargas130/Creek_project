      ...
      if (email) {
        try {
          ...
          if (res.data) {
            ...
            setDni(res.data.dni);
            setBirthday(res.data.birthday?.slice(0, 10)); // ajustar formato si es necesario
            setOcupation(res.data.ocupation);
            // Opcional: desactivar campos
          }
        } catch (error) {
...
<input type="text" value={phone} onChange={(ev) => setPhone(ev.target.value)} disabled={userExists} />
