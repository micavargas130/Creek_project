...
const New = () => {
  ...
  const fetchData = async () => {
    try {
      if (checkIn && checkOut) {
        ...
        // ✅ Solo autoasignar si el usuario ya eligió antes
        if (hasChosenLodge && res.data.length > 0) {
          const stillValid = res.data.find(l => l._id === lodgeId);
          if (!stillValid) {
            setLodgeId(res.data[0]._id); // autoasignar solo si la seleccion actual no está más disponible
          }
        }
      } else {
  ...
  useEffect(() => {
  if (lodgesInfo.length > 0) {
    // Si ya hay un lodgeId seleccionado y sigue disponible, no cambiarlo
    const currentLodgeStillAvailable = lodgesInfo.some(lodge => lodge._id === lodgeId);
    if (!currentLodgeStillAvailable) {
      setLodgeId(lodgesInfo[0]._id); // Selecciona la primera cabaña disponible
    }
  } else {
    setLodgeId(""); // No hay cabañas disponibles
  }
}, [lodgesInfo]);
              ...
              <div className="mb-64">
                <div>
                  ...
                  <select
                    value={lodgeId}
                    onChange={(e) => {
                      setLodgeId(e.target.value);
                      setHasChosenLodge(true); // ✅ Marca que el usuario eligió manualmente
                    }}
                  >
                    <option value="">Seleccioná una cabaña</option>
                    {lodgesInfo.map((lodge) => (
                ...
                <div>
                  <label>Cantidad de adultos</label> <input type="number" value={numberOfAdults} onChange={(ev) => setNumberOfAdults(ev.target.value)} />
                  <label>Cantidad de niños</label> <input type="number" value={numberOfChildren} onChange={(ev) => setNumberOfChildren(Number(ev.target.value) || 0)} />
                </div>
                <div>
