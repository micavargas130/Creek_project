...
const New = () => {
  ...
  async function newLodge(ev) {
    ...
    // Añadir la imagen al FormData si fue seleccionada
    if (file) {
      formData.append("photos", file);
    }
  
    try {
      const response = await axios.post("http://localhost:3000/lodges/", formData, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
      });
  
      const newLodgeData = response.data; 
      setLodgesInfo(prevLodges => [...prevLodges, newLodgeData]); 
