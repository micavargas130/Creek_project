...
const New = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get("/tents/occupiedPositions");
        const { occupiedPositions } = response.data;
        setOccupiedPositions(occupiedPositions);

        const lodgesResponse = await axios.get("/lodges");
  ...
  const newTent = async (ev) => {
    ...
    try {
      const tentResponse = await axios.post("/tents/createTents", {
        first_name,
        last_name,
        dni,
        email,
      ...
      const paymentData = {
        amount: 1000, // Asigna el monto adecuado según tu lógica de negocio
        type: "Ingreso",
        date: new Date().toISOString(),
        user: `${first_name} ${last_name}`,
        tent: tentId, // Asigna el ID de la carpa creada
        comment: "Carpa",
        status: "pendiente", // Cambiar a "pagada" si ya está pagada
      };
      ...
      alert("Error al registrar la carpa o accounting");
    }
  };
