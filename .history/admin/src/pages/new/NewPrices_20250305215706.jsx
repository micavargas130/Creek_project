...
const New = () => {
  ...
  useEffect(() => {
    const fetchPricesData = async () => {
      try {
        // Buscar el último precio registrado
        const lastCabanasRes = await axiosInstance.get("/prices/66f8b88d5e41d6ee37542fca");
        const lastCarpasRes = await axiosInstance.get("/prices/66a82bc2ac1709160e479670");
  ...
  const handleFormSubmit = async () => {
    try {
      await Promise.all([
        axiosInstance.put(`/prices/66f8b88d5e41d6ee37542fca`, formData.cabanas),
        axiosInstance.put(`/prices/66a82bc2ac1709160e479670`, formData.carpas),
      ]);
      setEditing(false);
    } catch (err) {
