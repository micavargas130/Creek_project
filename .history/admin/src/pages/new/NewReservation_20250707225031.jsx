  ...
  useEffect(() => {
    const fetchAvailableLodges = async () => {
      ...
      try {
        ...
        console.error("Error al obtener cabañas disponibles:", error);
      }
    };
        ...
        <div className="bottom">
          ...
          <div className="right">
            <form onSubmit={confirmFinishClick}>
              <div className="mb-64">
                <div>
                  ...
                  <select
                    ...
                    disabled={totalGuests === 0 || !checkIn || !checkOut}
                  >
                    <option value="" disabled>
                      {totalGuests === 0 ? "Primero ingresa el número de huéspedes": (!checkIn || !checkOut) ? "Selecciona fechas" : lodgesInfo.length === 0  ? "No hay cabañas disponibles"
              ...
              <button onClick={confirmActiveClick} disabled={loadingActivation}>
  {loadingActivation ? <CircularProgress size={20} /> : "Confirmar"}
</button>
            </form>
          </div>
        </div>
