  ...
  const fetchLodges = async () => {
    try {
      ...
      if (checkIn && checkOut) {
        ...
        res = await axiosInstance.get(`/lodges/available?checkIn=${checkIn}&checkOut=${checkOut}`);
      } else {
        console.log("checkIn", checkIn);
        console.log("checkOut",checkOut);
        res = await axiosInstance.get("/lodges");
      }
              ...
              <div className="mb-64">
                ...
                <div>
                 <label>Check-In</label>
                  <input type="date" value={checkIn} min={moment().format("DD-MM-DD")} 
                    onChange={(e) => {
                ...
                <div>
                  ...
                  <input
                    type="date"
                    value={checkOut}
                    min={checkIn || moment().format("YYYY-MM-DD")}
                    onChange={(e) => {
                      const selected = e.target.value;
                      const today = moment().format("YYYY-MM-DD");
                      if (selected < today) {
