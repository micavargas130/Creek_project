  ...
  useEffect(() => {
    const fetchPricesData = async () => {
      try {
        ...
        const cabanasPrices = pricesRes.data.filter(price => price.category === "cabanas");
        const carpasPrices = pricesRes.data.filter(price => price.category === "carpas");

        // Obtener los últimos precios de cabañas y carpas
        const lastCabanasPrice = cabanasPrices[cabanasPrices.length - 1] || {};
                    ...
                    <table className="priceHistoryTable">
                      ...
                      <tbody>
                        {priceHistory.map((price) => (
                          <tr key={price._id}>
                            <td>{new Date(price.date).toLconst [priceHistory, setPriceHistory] = useState([]);
  const [editing, setEditing] = useState(false);
