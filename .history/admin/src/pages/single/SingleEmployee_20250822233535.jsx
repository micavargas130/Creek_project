...
const fetchSchedules = async () => {
  ...
  const handleSelectEvent = async (event) => {
    ...
    const confirmDelete = window.confirm(
      `¿Seguro que quieres eliminar la tarea "${formatDate(event.task)}"?\n\nEsto eliminará todas las entradas de esta tarea desde el "${formatDate(event.start}" hasta el "${event.end}"`
    );
    if (confirmDelete) {
      try {
