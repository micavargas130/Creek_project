      ...
      try {
       ...
       let entityId;
        ...
        if (entityType === "lodge") {
         ...
         const historyResponse = await axiosInstance.get(`/accounting/history/${bookingData._id}`);
              ...
              <table className="paymentHistoryTable">
                ...
                {payment.receipt && payment.receipt.length > 0 ? (
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    <img
                      src={`http://localhost:3000/${payment.receipt[0]}`}
                      alt="Comprobante"
                    ...
                    <DeleteForeverIcon
                      onClick={async () => {
                        const filename = payment.receipt[0].split("/").pop();
                        try {
                          await axiosInstance.delete(`/accounting/pay/receipt/${payment._id}/${filename}`);
                          singleObserver.notify("paymentCompleted", { accountingId });
                        } catch (error) {
