...
const Single = () => {
  ...
  useEffect(() => {
    axiosInstance.get(`/lodges/${lodgeId}`)
      .then((response) => {
    ...
    axiosInstance.get(`/user/${booking.user._id}`)
      .then((userResponse) => {
        ...
        const currentDate = new Date();
               ...
               // Calcula la fecha de check-out         const checkOutDate = new Date(booking.checkOut);
 // Compara las fechas para determinar si es el día de check-out        if (currentDate >= checkOutDate && !notificationCreated) {   // Si es el día de check-out, crea la notificación   axiosInstance.post("/notifications", {          type: "Fin de estadia",          cabain: lodgeId, // Cambia "cabain" por "cabin" si es un error tipográfico          client: booking.user,          date: checkOutDate,            });
