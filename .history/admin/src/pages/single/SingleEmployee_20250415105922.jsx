...
const Single = () => {
  ...
  const fetchSchedules = async () => {
    try {
      const res = await axiosInstance.get("/schedule"); // o por empleado
  ...
  const handleAddTask = async () => {
    ...
    try {
      ...
      // Crear solo una entrada en la base de datos con la info del rango
      ...
      while (current <= end) {
        const currentDay = current.getDay(); // 0 (domingo) - 6 (sábado)
        
        if (!repeatWeekly || currentDay === new Date(startDate).getDay()) {
          const formattedDate = current.toISOString().split("T")[0];
          expandedEvents.push({
            id: createdSchedule._id,
            title: createdSchedule.task,
        ...
        <div className="bottomSection">
          ...
          <div className="taskSection">
            <div className="addTaskForm">
              ...
              <input type="date" placeholder= "Inicio" value={startDate} onChange={(e) => setStartDate(e.target.value)} />
