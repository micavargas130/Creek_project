...
const Single = () => {
  ...
  return new Date(year, month - 1, day); // sin UTC, hora local 00:00
};
...
const end = parseLocalDate(schedule.endDate);
      ...
      for (
        ...
        events.push({
          ...
          start: new Date(date.setHours(0,0,0,0)), // 👈 normalizar inicio
          end: new Date(date.setHours(23,59,59,999)), // 👈 normalizar fin
          allDay: true,
        });
      }
  ...
  const handleSelectEvent = async (event) => {
    ...
    const confirmDelete = window.confirm(
      `¿Seguro que quieres eliminar la tarea "${event.task}"?\n\nEsto eliminará todas las entradas de esta tarea desde el "${event.start}" hasta el "${event.endDate}"`
    );
    if (confirmDelete) {
      try {
