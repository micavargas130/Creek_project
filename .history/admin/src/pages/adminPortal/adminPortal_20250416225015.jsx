import {
  ...
  {reservasPorMes.map((item) => (
    <div key={item.fecha} className="bg-white p-4 rounded-xl shadow">
      <h3 className="text-lg font-bold">{item.fecha}</h3>
      <p>Cabañas: {item.cabañas}</p>
      <p>Carpas: {item.carpas}</p>
      <p>Total: {item.cabañas + item.carpas}</p>
    </div>
  ))}
</div>
