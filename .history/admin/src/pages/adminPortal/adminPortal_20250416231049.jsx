          ...
          <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="mb-4">
              ...
              {vista === "dia" && (
                <input
                  type="date"
                  className="border rounded px-2 py-1 mt-2 md:mt-0 ml-2"
...
{/* Ranking de reservas por cabaña */}
<div className="bg-white rounded-2xl p-4 shadow">
  <h2 className="text-lg font-semibold mb-2">Ranking de Reservas por Cabaña</h2>
  <table className="w-full table-auto border mt-4">
    <thead className="bg-gray-100">
      <tr>
    ...
    <tbody>
      {reservasPorCabaña.map((item, index) => (
        <tr key={item._id}>
          <td className="border px-4 py-2">{index + 1}</td> {/* Ranking */}
          <td className="border px-4 py-2">{item.name}</td> {/* Nombre de la cabaña */}
          <td className="border px-4 py-2">{item.total}</td> {/* Número de reservas */}
        </tr>
      ))}
    </tbody>
  </table>
