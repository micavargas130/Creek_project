import {
  ...
  const Single = () => {
    ...
    useEffect(() => {
      ...
      fetchData();
    }, [vista, fechaSeleccionada, mesRanking, añoRanking]);
      ...
      <Box sx={{ p: 3, display: 'grid', gap: 3, gridTemplateColumns: { xs: "1fr", md: "1fr 1fr" } }}>
        ...
        {/* Ranking por cabaña */}
         <Paper sx={{ p: 2 }}>
          ...
          {/* Título */}
          <Typography variant="h6" gutterBottom>Ranking de Cabañas más Reservadas</Typography>
          ...
          <TableContainer>
           <Table>
             <TableHead>
               <TableRow>
                 <TableCell>Puesto</TableCell> {/* Nueva columna */}
             ...
             <TableBody>
               {reservasPorCabaña.map((item, index) => (
                 <TableRow key={item.lodgeId}>
                   <TableCell>{index + 1}°</TableCell> {/* Ranking numérico */}
                   <TableCell>{item.lodgeName}</TableCell>
