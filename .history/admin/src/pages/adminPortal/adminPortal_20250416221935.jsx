  ...
  const Single = () => {
    ...
    useEffect(() => {
      const fetchData = async () => {
        try {
            const res1 = await axiosInstance.get("/graphs/reservas-tiempo", { groupBy: vista });
          ...
          const formatearFecha = (_id) => {
            const año = _id.year;
            const mes = _id.month?.toString().padStart(2, '0');
            const dia = _id.day?.toString().padStart(2, '0');
          ...
          const agregarReserva = (data, tipo) => {
            data.forEach(item => {
              const clave = formatearFecha(item._id); // ✅ usar _id del group
              if (!clave) return;
              ...
              agrupado[clave][tipo] += item.total;
            });
          };
