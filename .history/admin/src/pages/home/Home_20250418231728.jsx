    ...
    <div className="topSection">
      <div className="calendarSection">
        <div className="calendarHeader">
          ...
          <button onClick={() => setShowModal(true)} className="addTaskBtn">+ Agregar tarea</button>
        </div>
    ...
    {showModal && (
      <div className="modal">
        <div className="modalContent">
          <h3>Agregar tarea</h3>
          <div className="formGroup"><label htmlFor="task">Título:</label><input id="task" type="text" value={task} onChange={(e) => setTask(e.target.value)} /></div>
          <div className="formGroup"><label htmlFor="startDate">Fecha Inicio:</label><input id="startDate" type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)} /></div>
          <div className="formGroup"><label htmlFor="startTime">Hora inicio:</label><input id="startTime" type="time" value={startTime} onChange={(e) => setStartTime(e.target.value)} /></div>
          <div className="formGroup"><label htmlFor="endDate">Fecha Fin:</label><input id="endDate" type="date" value={endDate} onChange={(e) => setEndDate(e.target.value)} /></div>
          <div className="formGroup"><label htmlFor="endTime">Hora fin:</label><input id="endTime" type="time" value={endTime} onChange={(e) => setEndTime(e.target.value)} /></div>
          <div className="formGroup"><label><input type="checkbox" checked={repeatWeekly} onChange={(e) => setRepeatWeekly(e.target.checked)} /> Repetir semanalmente</label></div>
          <div className="formActions"><button onClick={handleAddTask}>Guardar</button><button onClick={() => setShowModal(false)}>Cancelar</button></div>
        </div>
      </div>
    )}
  </div>
