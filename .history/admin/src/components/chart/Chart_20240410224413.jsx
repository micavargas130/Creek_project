...
const Chart = ({ title }) => {
  ...
  const closeNotification = async (notificationId) => {
    try {
      // Realiza una solicitud al servidor para actualizar el estado de la notificación
      await axios.put(`/notifications/${notificationId}`, { closed: true });
      // Actualiza el estado local de las notificaciones para reflejar el cambio
      setNotifications(prevNotifications =>
        prevNotifications.map(notification =>
          notification._id === notificationId ? { ...notification, closed: true } : notification
        )
      );
    } catch (error) {
      ...
      <div className="notification-grid">
        {notifications.map((notification) => (
          ...
          >
            ...
            <CloseIcon className="cencelIcon" onClick={() => closeNotification(notification._id)} />
          </div>
        ))}
