...
useEffect(() => {
  const fetchData = async () => {
    const response = await axiosInstance.get(`/api/accounting/balance`, {
      params: {
        year: selectedYear,
  ...
  fetchData();
    ...
    const renderChart = () => {
      switch (activeTab) {
        case "evolucion":
          return <Chart aspect={4 / 1} title="Evolución del Dinero" data={processedData} selectedYear={selectedYear} selectedMonth={selectedMonth}/>;
