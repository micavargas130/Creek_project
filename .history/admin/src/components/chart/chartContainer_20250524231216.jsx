...
const ChartContainer = ({ rawData, processedData, totalMoney, totalIncome, totalExpense }) => {
...
useEffect(() => {
  const fetchData = async () => {
    try {
      const res = await axios.get(`/api/accounting/evolution?year=${selectedYear}&month=${selectedMonth}`);
  ...
  fetchData();
    ...
    const renderChart = () => {
      switch (activeTab) {
        case "evolucion":
       return <Chart aspect={4 / 1} title="Evolución del Dinero" data={chartData} selectedYear={selectedYear} selectedMonth={selectedMonth}/>;
