...
const Featured = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        //trae la posicion que ocupa cada cabaña
        ...
        {Array.from({ length: 8 }, (_, row) => (
          <div key={row} className="map-row">
            {Array.from({ length: 10 }, (_, col) => {
              ...
              console.log("Lodge Positions:", lodgeAtPosition);
              return (
                <div
                  key={col}
                  id={`${row}-${col}`}
                  ...
                  } ${lodgesInfo.state === "Desocupada" ? "vacant" : ""} ${selectedCell.includes(`${row}-${col}`) ? "selected" : ""}`}
