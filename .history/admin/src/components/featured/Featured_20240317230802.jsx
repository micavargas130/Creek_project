...
const Featured = () => {
  ...
  const getIconType = (row, col) => {
    ...
    // Verificar si la cabaña está ocupada y es del tipo HouseSidingRoundedIcon
    const isLodgeOccupied = lodgeState === "ocupada" && iconType === "HouseSidingRoundedIcon";
          ...
          const isLodgeCell = iconType === "HouseSidingRoundedIcon";
              ...
              return (
                <div
                  ...
                  id={`${row}-${col}`}
                  className={`map-cell ${
                   isLodgeCell && isCellOccupied ? "occupied" : ""
                  } ${lodgeState} ${selectedCell.includes(`${row}-${col}`) ? "selected" : ""}`}
