...
const Featured = () => {
  ...
  const getIconType = (row, col) => {
    ...
    const lodgeAtPosition = lodgesInfo.find(lodge => lodge.location.row === row && lodge.location.col === col);

    // Verificar si hay una cabaña en esta posición
    if (lodgeAtPosition) {
      // Si la cabaña está ocupada, se puede devolver el icono correspondiente
      // Si el estado de la cabaña indica que está disponible, se puede devolver un color específico o un estilo CSS
      // Si el estado de la cabaña indica que está reservada, se puede devolver otro color o estilo
      // Y así sucesivamente, dependiendo de los diferentes estados de las cabañas
      if (lodgeAtPosition.status === 'disponible') {
        // Devolver el estilo para una cabaña disponible (por ejemplo, verde)
        return "green";
      } else {
        // Devolver el estilo para una cabaña en otro estado (por ejemplo, rojo)
        return "red";
      }
    }
            ...
            {Array.from({ length: 10 }, (_, col) => {
              ...
              return (
                <div
                  ...
                  } ${lodgeAtPosition.state === "Desocupada" ? "vacant" : ""} ${selectedCell.includes(`${row}-${col}`) ? "selected" : ""}`}
