  ...
  const actionColumn = [
    {
      ...
      renderCell: (params) => {
        ...
        console.log(params.row.totalAmount)
  ...
  const namesColumns = [
    {
        ...
        valueGetter: (params) => {
          ...
          if(params.row.lodge) return ((params.row.user?.first_name + " " + params.row.user?.last_name)|| "N/A" )// Cambia `name` al campo correcto si es distinto
          if(params.row.tent) return ((params.row.tent?.first_name + " " + params.row.tent?.last_name ) || "No name")
    },
    {
        field: "entity",
        headerName: "Entidad",
...
<Dialog open={isStatusDialogOpen} onClose={closeStatusOptions}>
  ...
  <DialogActions>
    ...
    <Button
      ...
      onClick={async () => {
        try {
          // Envía la actualización al servidor para marcar como pagada
          await axios.put(`http://localhost:3000/accounting/status/${selectedParams.row._id}`, {
            status: "pagada",
            amountPaid: selectedParams.row.totalAmount, // Completa el pago
          });
          closeStatusOptions();
          window.location.reload();
        } catch (error) {
...
<Dialog open={isStatusDialogOpen} onClose={closeStatusOptions}>
  ...
  <DialogActions>
    <Button onClick={closeStatusOptions} color="primary">
      Cancelar
    </Button>
    <Button onClick={() => handleStatusChange(selectedParams)} color="primary">
      Guardar
    </Button>
  </DialogActions>
</Dialog>
