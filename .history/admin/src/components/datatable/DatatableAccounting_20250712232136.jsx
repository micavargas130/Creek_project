...
const handleCommentSubmit = async () => {
  ...
  const chartData = data.filter((entry) => (entry.type !== "Ingreso" || entry.status!== "686f099aacd6f16c7fd3c905"))
    .map((entry) => ({
      name: entry.date,
      Total: entry.amount,
    }));
  ...
  return data
    .filter((entry) => (entry.type !== "Ingreso" || entry.status!== "686f099aacd6f16c7fd3c905"))
    .map((entry) => {
      accumulatedTotal += entry.type === "Ingreso" ? entry.amount : -entry.amount;
      console.log("entry", entry);
      console.log("accoumulates", accumulatedTotal);
      return {
        name: entry.date,
    ...
    renderCell: (params) => {
      const handleCancelClick = async () => {
        try {
          ...
          await axiosInstance.post(`/accounting/return/${selectedParams.row._id}`, {
      ...
      const dontShowReturnButton = ["devolución"].includes(params.row.status.status);
