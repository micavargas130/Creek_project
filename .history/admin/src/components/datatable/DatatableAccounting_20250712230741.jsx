...
const handleCommentSubmit = async () => {
  ...
  const chartData = data.filter((entry) => (entry.type !== "Ingreso" || entry.status.status!== "cancelado"))
    .map((entry) => ({
      name: entry.date,
      Total: entry.amount,
    }));
...
console.log("chartData", chartData)
  ...
  return data
    .filter((entry) => (entry.status?.status !== "cancelado"))
    .map((entry) => {
      accumulatedTotal += entry.type === "Ingreso" ? entry.amount : -entry.amount;
      console.log("accoumulates", entry);
