...
const Datatable = () => {
  ...
  //FUNCION PARA COBRARLE AL CLIENTE (abre el modal de cobro)
  const handleArrivalClick = async (params) => {
    try {
      ...
      if (!lodgeStatus || !lodgeStatus.status) {
        ...
        lodgeStatus.status.status === "ocupado" || lodgeStatus.status.status === "mantenimiento"
      ) {
      ...
      renderCell: (params) => {
        ...
        return (
          <div className="cellAction">
            ...
            <div className="viewButton" onClick={() => handleInfoClick(params.row)} >
              Info
            </div>
  ...
  //le paso los datos planos al datatable para poder filtrarlos
        ...
        <div className="modal">
          <div className="modalContent">
            ...
            <button
               onClick={() => {
                 confirmActiveClick();
                 // No cierres el modal acá directamente, lo hace `finally` al terminar el proceso
                 setShowDepositSummary(false);
               }}
               disabled={loadingActivation} // deshabilita para evitar múltiples clics
             >
               {loadingActivation ? "Procesando..." : "Confirmar"}
             </button>
