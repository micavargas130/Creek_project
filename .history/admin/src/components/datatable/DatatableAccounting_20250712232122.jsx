...
const handleCommentSubmit = async () => {
  ...
  const chartData = data.filter((entry) => (entry.type !== "Ingreso" || entry.status.status!== "686f099aacd6f16c7fd3c905"))
    .map((entry) => ({
      name: entry.date,
      Total: entry.amount,
    }));
  ...
  return data
    .filter((entry) => (entry.type !== "Ingreso" || entry.status?.status !== "cancelado"))
    .map((entry) => {
