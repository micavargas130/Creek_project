  ...
  const actionColumn = [
    {
      ...
      const accountingId = axios.put(`http://localhost:3000/accounting/history/:lodgeId${params.row._id}`);
      const isOccupied = params.row.state && params.row.state.status === "ocupado";
      const isPaid = params.row.occupiedBy && params.row.occupiedBy.paymentStatus === "pagada";
