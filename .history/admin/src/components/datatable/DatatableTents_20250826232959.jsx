...
const Datatable = () => {
  ...
  useEffect(() => {
    ...
    globalObserver.subscribe("changesTents", handleStatusChange);
   
    const fetchAccountingRecords = async () => {
      try {
        const response = await  axiosInstance.get("/accounting"); //axiosIntance
    ...
    return () => globalObserver.unsubscribe("changesTents", handleStatusChange);
  }, []);
  ...
  console.log("sorted", tentsWithPaymentStatus)
  const sortedTents = [...tentsWithPaymentStatus].sort((a, b) => b._id.localeCompare(a._id));
  ...
  const confirmFinishClick = async (params) => {
    try {
      ...
      globalObserver.notify("changesTents");
    } catch (error) {
        ...
        return (
          <div className="cellAction">
           {isPending && (
            ...
            <div 
              className={`arrivalButton ${isPending ? "disabledButton" : ""}`} 
