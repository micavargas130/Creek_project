...
const Datatable = () => {
  ...
  useEffect(() => {
    ...
    const handleReservaSocket = ({ booking }) => {
      console.log("Reserva actualizada por socket:", booking);
      setBookings((prev) =>
        prev.map((b) => (b._id === booking._id ? booking : b))
      );
    };
  
    socket.on("reserva-actualizada", handleReservaSocket);
    socket.on("reserva-completada", handleReservaSocket);
    socket.on("reserva-activada", handleReservaSocket); // <- ESTA FALTABA
  
    return () => {
      socket.off("reserva-actualizada", handleReservaSocket);
      socket.off("reserva-completada", handleReservaSocket);
      socket.off("reserva-activada", handleReservaSocket); // <- Y ESTO TAMBIÉN
    };
  }, []);
  ...
  const confirmFinishClick = async () => {
    ...
    try {
      ...
      console.log("existing", existingAccounting)
  ...
  const handleInfoClick = async (row) => {
    ...
    if (currentStatus === "Activa") {
      try {
        const res = await axiosInstance.get(`/api/accounting/byBooking/${row._id}`);
