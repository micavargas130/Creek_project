...
const Datatable = () => {
  ...
  const filteredData1 = data.filter(item =>  item.status && (item.status.status === "Completada" || item.status.status === "Cancelada"));


  useEffect(() => {
    // Filtrar las reservas para mostrar solo las que tienen estado "completado" o "cancelado"
    if (data && data.length > 0) {
      const filtered = data.filter(item =>  item.status && (item.status.status === "Completada" || item.status.status === "Cancelada"));
