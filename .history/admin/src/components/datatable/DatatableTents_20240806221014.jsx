...
const Datatable = () => {
  ...
  try {
    const paymentStatus = window.prompt("Seleccionar estado del pago: Pagada, Parcial, Pendiente", "Pendiente");

    if (!["Pagada", "Parcial", "Pendiente"].includes(paymentStatus)) {
    ...
    const newAccountingResponse = await axios.post('/accounting', paymentData);
    const newAccounting = newAccountingResponse.data;

    const newStatus = {
      status: paymentStatus.toLowerCase(), // Convertimos el estado a minúsculas
