...
const Datatable = () => {
  ...
  useEffect(() => {
    const fetchAccountingRecord = async () => {
      try {
        const response = await axios.get(`/accounting/${tentId}`);
        const accountingRecord = response.data;
        

        if (!accountingRecord) {
          setError('No se encontraron registros de pago para esta tienda');
          setStatusData(null);
        } else {
          setStatusData(accountingRecord);
        }

        console.log("response", accountingRecord);
      } catch (error) {
        console.error('Error al obtener el estado de pago:', error);
        setError('Error al obtener el estado de pago');
      }
    };
