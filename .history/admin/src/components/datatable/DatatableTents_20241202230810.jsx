...
const Datatable = () => {
  ...
  useEffect(() => {
   ...
   const tentsWithPaymentStatus = filteredData.map((tent) => {
    const relatedAccounting = accountingData.find((record) => String(record.tent._id) === String(tent._id));
    const isPending = relatedAccounting ? relatedAccounting.remainingAmount > 0 : true;
    console.log("Tent ID:", tent._id);
console.log("Related Accounting Record:", relatedAccounting);
console.log("Remaining Amount:", relatedAccounting?.remainingAmount);
    return {
      ...tent,
  ...
  return (
    <div className="datatable">
      ...
      <DataGrid
        className="datagrid"
        rows={filteredData}
        columns={[...userColumns, ...paymentStatusColumn, ...guestNumberColumn, ...actionColumn]}
        pageSize={9}
