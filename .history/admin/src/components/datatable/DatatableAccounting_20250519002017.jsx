...
useEffect(() => {
  fetchData();
  const handleAccountingUpdate = (event) => {
    setData(event.detail); // o lo que quieras hacer con los nuevos datos
  };

  window.addEventListener("accountingChange", handleAccountingUpdate);

  return () => {
    window.removeEventListener("accountingChange", handleAccountingUpdate);
  };
}, []);
