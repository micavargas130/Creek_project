...
useEffect(() => {
 ...
 const openStatusOptions = (params) => {
  ...
  const handleCommentClick = (params) => {
  const newComment = window.prompt("Ingrese su comentario:");
  if (!newComment) return;

  axiosInstance
    .patch(`/accounting/${params.row._id}/comment`, { comment: newComment })
    .then(() => {
      alert("Comentario agregado");
      globalObserver.notify("accountingChange"); // Recargar si es necesario
    })
    .catch((error) => {
