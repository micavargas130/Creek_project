...
import accountingObserver from "../../utils/observer.js";
...
useEffect(() => {
  fetchData();
  accountingObserver.subscribe("accountingChange", fetchData); 
  return () => {
    accountingObserver.unsubscribe("accountingChange", fetchData); 
  };
}, []);
...
useEffect(() => {
    ...
    socket.on("reserva-actualizada", ({ booking }) => {
      console.log("Reserva actualizada por socket:", booking);

      // Actualizar solo la reserva cambiada
      setBookings((prev) =>
        prev.map((b) => (b._id === booking._id ? booking : b))
      );
    });

    return () => socket.off("reserva-actualizada");
  }, []);
 ...
 const openStatusOptions = (params) => {
  ...
  const handleCommentSubmit = async () => {
    ...
    try {
      ...
      closeDialog();
      accountingObserver.notify("accountingChange");
    } catch (error) {
  ...
  const actionColumn = [
    {
      ...
      renderCell: (params) => {
        ...
        const showChangeStatusButton = ["parcial",  "seña"].includes(params.row.status.status);
