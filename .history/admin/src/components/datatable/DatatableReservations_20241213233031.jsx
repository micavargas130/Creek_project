...
const Datatable = () => {
  ...
  const handleArrivalClick = async (params) => {
    ...
    setAmount(params.row.totalAmount);
  ...
  const confirmFinishClick = async () => {
    ...
    try {
      ...
      setAmountToPay((booking.totalAmount * daysDifference));
      ...
      if (selectedStatus === "pagada") {
        payment = ((booking.totalAmount) * amountDays); // Pago completo
      ...
      const paymentData = {
        amount: payment,
        totalAmount: (booking.totalAmount)*amountDays,
        type: "Ingreso",
        date: new Date().toISOString(), // Fecha actual
        user: booking.user._id,
        lodge: booking._id, //en lodge guardo booking 
        remainingAmount:  (booking.totalAmount - partialPayment),
        comment: "",
