...
import lodgeObserver from "../../utils/observers/lodgeObserver.js";
...
const Datatable = (props) => {
  ...
  useEffect(() => {
    const handleStatusChange = () => {
      refetch(); // 👈 vuelve a pedir los datos de cabañas
    };
  
    lodgeStatusObserver.subscribe(handleStatusChange);
    return () => lodgeStatusObserver.unsubscribe(handleStatusChange);
  }, []);

  //para cuando se elige el estado "mantenimiento"
  const handleMaintenanceSubmit = (params) => {
    try {
      axiosInstance.put(`http://localhost:3000/lodges/comment/${params.row._id}`, {
  ...
  const actionColumn = [
    {
      ...
      renderCell: (params) => {
        ...
        const handleLiberar = async () => {
          ...
          try {
            await axiosInstance.put(`lodges/eliminarComment/${params.row._id}`);
