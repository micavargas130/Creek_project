...
const actionColumn = [
  {
    ...
    renderCell: (params) => {
      ...
      // ⚠️ Verifica si el lodge está cancelado (por ID de estado)
      ...
      return (
        <div className="cellAction">
          {isLodgeCanceled && (
            <div
              title="Este registro está vinculado a una cabaña cancelada"
...
const alertColumn = [
  {
    ...
    width: 50,
    renderCell: (params) => {
      // si es una cabaña con estado "Cancelada"
      console.log("quesesto", params.row.lodge.status)
      if (params.row.lodge?.status === "67affcc10cb373c285f445eb") {
        return (
          <div title="Registro vinculado a una cabaña cancelada" style={{ color: "red", fontWeight: "bold" }}>
            !
          </div>
        );
      }
  ...
  return (
    <div className="datatable">
      ...
      <DataGrid
        className="datagrid"
        rows={data}
        columns={[...userColumns, ...namesColumns, ...actionColumn, ...alertColumn]}
        pageSize={9}
          ...
          <Button color="primary" onClick={async () => {
             try {
               ...
               await axiosInstance.post(`http://localhost:3000/accounting/pay/${selectedParams.row._id}`, {
                ...
                closeStatusOptions();
                      globalObserver.notify("accountingChange"); // Recargar para mostrar cambios
               } catch (error) {
