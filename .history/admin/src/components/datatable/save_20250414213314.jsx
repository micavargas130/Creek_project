...
const Datatable = () => {
  ...
  const handleArrivalClick = async (params) => {
    try {
      ...
      const lodge = await axiosInstance.get(`/lodges/${lodgeId}`);
      console.log("lodge", lodge)
      const  lodgeStatus  = lodge.data.state
      console.log("lodgeStatus", lodgeStatus)
      // Si la cabaña está ocupada o en mantenimiento, no permitir avanzar
      if (lodgeStatus.status === "ocupado" || lodgeStatus.status=== "mantenimiento") {
        alert("No se puede proceder con el cobro porque la cabaña está ocupada o en mantenimiento.");
        return;
      }
        // Si la cabaña está disponible, continuar con el flujo normal
