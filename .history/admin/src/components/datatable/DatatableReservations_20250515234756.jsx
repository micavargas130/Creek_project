...
const Datatable = () => {
  ...
  const handleInfoClick = async (row) => {
    ...
    if (currentStatus === "Activa") {
      ...
      if (lodgeId) {
        navigate(`/booking/${lodgeId}`);
      } else {
        alert("No se encontró la cabaña asociada.");
      }
    } else {
      alert("Solo se puede acceder a reservas activas o pendientes.");
    }
  };
  ...
  const actionColumn = [
    {
      ...
      renderCell: (params) => {
        ...
        const isActive = params.row.status.status === "Activa";
  
        return (
          <div className="cellAction">
            <div
              className="viewButton"
              style={{ cursor: isActive ? "pointer" : "not-allowed", color: isActive ? "blue" : "gray" }}
