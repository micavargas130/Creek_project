...
const Datatable = (props) => {
  const { data, loading, error, refetch } = useFetch("/accounting");
...
const fetchData = async () => {
  console.log("[fetchData] Ejecutando...");
  try {
    setLoading(true);
    const res = await axiosInstance.get("/accounting");
    console.log("[fetchData] Nuevos datos:", res.data);
    setData(res.data);
  } catch (error) {
...
useEffect(() => {
  ...
  console.log("[Observer] Cambio en contabilidad detectado, recargando...");
  refetch();
};
    accountingObserver.subscribe("accountingChange", handleAccountingChange);

    return () => {
      accountingObserver.unsubscribe("accountingChange", handleAccountingChange);
    };
  }, []);
