...
const MapComponent = ({ onCellClick, lodgesInfo }) => {
  ...
  const [hoveredLodgeName, setHoveredLodgeName] = useState(null);
  ...
  const handleCellMouseEnter = (row, col) => {
    ...
    if (lodge) {
      setHoveredLodgeName(lodge.name); // Mostrar nombre de la cabaña
    }
  };

  const handleCellMouseLeave = () => {
    setHoveredLodgeName(null); // Ocultar nombre al quitar hover
  };
  ...
  const getIconType = (row, col) => {
    ...
    if (lodge) {
      return { 
        ...
        lodgeState: lodge.latestStatus || "desocupada",
      };
    }
            ...
            {Array.from({ length: 10 }, (_, col) => {
              ...
              return (
                ...
                >
                  ...
                  {hoveredLodgeName && (
                    <div className="lodge-name-tooltip">
                      {hoveredLodgeName}
                    </div>
                  )}
                </div>
