      ...
      lodges.map(async (lodge) => {
        if (lodge.occupiedBy) {
          ...
          const accounting = await Accounting.findOne({
            ...
            .populate('status', 'name') // Usar populate para traer el nombre del estado desde Status
            .select('remainingAmount totalAmount status');
          ...
          return {
            ...
            paymentInfo: accounting
              ? {
                  ...
                  paymentStatus: accounting.status?.name || null, // Extraer el nombre del estado
                }
