...
export const getAccounting = async (req, res, next) => {
    try {
      const accounting = await Accounting.findById(req.params.id).populate('status')..populate({
        ...
        populate: { // Popula el campo lodge dentro de booking
          path: 'lodge', // Campo del modelo Booking que hace referencia al modelo Lodge
          select: 'name', // Especifica los campos del modelo Lodge que deseas traer
        },
