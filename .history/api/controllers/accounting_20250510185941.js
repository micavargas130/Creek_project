...
export const getPaymentHistoryByLodge = async (req, res, next) => {
  ...
  export const deleteReceipt = async (req, res, next) => {
    ...
    try {
      ...
      const updatedReceipts = payment.receipt.filter((r) => r !== `receipts/${filename}`);
      payment.receipt = updatedReceipts;
      await payment.save();
      ...
      res.status(200).json({ message: "Comprobante eliminado correctamente" });
    } catch (error) {
      next(error);
    }
  };
