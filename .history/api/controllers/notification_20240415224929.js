...
export const getClosedNotification = async (req, res) => {
    ...
    console.log("Received userId:", userId); // Imprime el userId recibido

    try {
        const notifications = await Notifications.find({
            closedBy: { $ne: userId } 
        });
        console.log("Notifications found:", notifications); // Opcional: Imprimir las notificaciones encontradas
        res.send(notifications);
    } catch (error) {
        console.error("Error fetching notifications:", error); // Usa console.error para errores
