...
import Price from "../models/Prices.js"; // asegurate de importar el modelo de precios
...
export const getAccountings = async (req, res, next) => {
    try {
        const accounting = await Accounting.find().populate('status').populate({
          ...
          populate: {
            path: 'user',
            select: 'first_name last_name', // Podés incluir más campos si los necesitás
          },
        }).populate('tent');
...
export const getPaymentHistoryByLodge = async (req, res, next) => {
  export const getPaymentHistoryByBooking = async (req, res, next) => {
    try {
      const { bookingId } = req.params;
  
      // Buscar el registro de contabilidad relacionado con esa reserva (booking)
      const accounting = await Accounting.findOne({ lodge: bookingId });
  
      if (!accounting) {
        return res.status(404).json({ message: "No se encontró un registro contable para esta reserva." });
      }
  
      // Buscar el historial de pagos relacionado al registro de contabilidad
      const paymentHistory = await PaymentHistory.find({ accounting: accounting._id })
        .populate("status") // opcional: trae los datos del estado
        .sort({ date: 1 });
