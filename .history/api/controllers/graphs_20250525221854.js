...
export const getReservationsOverTime = async (req, res) => {
  ...
  try {
   ...
   if (vista === "mes") {
  if (fecha) {
    const fechaInicio = new Date(fecha); // fecha = "2025-05-01"
    fechaInicio.setHours(0, 0, 0, 0); // Aseguramos que sea 00:00:00

    const fechaFin = new Date(fechaInicio);
    fechaFin.setMonth(fechaFin.getMonth() + 1); // Primer día del mes siguiente
    fechaFin.setHours(0, 0, 0, 0); // Aseguramos que sea 00:00:00

    match.checkIn = { $gte: fechaInicio, $lt: fechaFin }; // Usa $lt, no $lte
    groupBy.year = { $year: "$checkIn" };
    groupBy.month = { $month: "$checkIn" };
  }
}
