...
export const createLodge = async (req, res, next) => {
  ...
  const file = req.file; // Accede al archivo adjunto procesado por multer

  try {
    // Aquí puedes guardar la información de la cabaña y la ruta del archivo en tu base de datos
    // También puedes realizar otras operaciones con el archivo, como moverlo a otra ubicación, etc.
    res.status(200).json({ message: 'Cabaña creada exitosamente', file });
  } catch(err) {
    next(err);
  }
};
