...
export const getAccountingByTent = async (req, res, next) => {
  try {
    const { tentId } = req.params;
    console.log("tent", tentId);

    // Busca la contabilidad relacionada con la reserva de carpa específica
    const accounting = await Accounting.findOne({ tent: tentId })
      .populate('status')
      .populate('lodge')
      .populate('tent');

    if (!accounting) {
      return res.status(404).json({ message: "No se encontró un registro de contabilidad para esta reserva de carpa." });
    }
