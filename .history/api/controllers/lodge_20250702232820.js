...
// GET /lodges/available?checkIn=2025-10-10&checkOut=2025-10-13
router.get('/lodges/available', async (req, res) => {
  const { checkIn, checkOut } = req.query;
  const lodges = await Lodge.find(); // o aplicar un filtro adicional si tenés cabañas deshabilitadas

  const overlappingBookings = await Booking.find({
    $or: [
      { checkIn: { $lt: checkOut }, checkOut: { $gt: checkIn } },
    ],
    status: { $ne: "Cancelada" }, // si usás texto o ID, ajustalo
  });
