...
export const getAccountings = async (req, res, next) => {
    try {
        const accounting = await Accounting.find().populate('status').populate('user', ['first_name','last_name']).populate('lodge', 'name');
...
export const getPaymentHistoryByLodge = async (req, res, next) => {
    try {
        ...
        // Filtra los pagos asociados al ID de la reserva (almacenado en 'lodge')
        const payments = await Accounting.find({ lodge: lodgeId }).sort({ date: 1 });
