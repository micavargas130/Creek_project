...
const getLodges = async (req, res) => {
  try {
    ...
    // Agregamos información de pagos a cada cabaña, buscando en Accounting
    const lodgesWithPaymentInfo = await Promise.all(
      lodges.map(async (lodge) => {
        if (lodge.occupiedBy) {
          const accounting = await Accounting.findOne({
            lodge: lodge.occupiedBy._id, // Buscamos en Accounting usando el ID de la reserva
          }).select('remainingAmount totalAmount');

          return {
            ...lodge.toObject(),
            paymentInfo: accounting || null, // Agregamos la información de pagos si existe
          };
        }
