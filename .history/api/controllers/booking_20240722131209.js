...
export const getBookings = async (req, res, next) => {
  try {
    const bookings = await Booking.find({})
      .populate('user', 'first_name last_name') // Aquí populamos solo el nombre y apellido
      .populate('status', 'status'); // Asegúrate de también popular el estado si es un objeto referenciado
...
export const getBooking = async (req, res, next) => {
  try {
    const booking = await Booking.findById(req.params.id).populate('status'); // Populate para obtener el estado completo
...
export const deleteBookingByLodge = async (req, res, next) => {
  try {
    const booking = await Booking.findOneAndDelete({ place: req.params.lodgeId }).populate('status'); // Populate para obtener el estado completo
    res.status(200).json(booking);
  } catch (err) {
    next(err);
  }
