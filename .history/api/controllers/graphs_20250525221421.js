...
export const getReservationsOverTime = async (req, res) => {
  ...
  try {
    ...
    if (vista === "mes") {
  if (fecha) {
    const fechaInicio = new Date(fecha); // 2025-05-01
    const fechaFin = new Date(fechaInicio);
    fechaFin.setMonth(fechaFin.getMonth() + 1); // 2025-06-01

    match.checkIn = { $gte: fechaInicio, $lt: fechaFin }; // Usa $lt en lugar de $lte
  }
