...
import Lodges from '../models/Lodges.js'; // Asegúrate de importar el modelo de cabaña correctamente

export const createLodge = async (req, res, next) => {
    try {
        // Extrae los campos del formulario de la solicitud
        const { name, description, capacity, services } = req.body;
        
        // Verifica si hay una foto adjunta en la solicitud
        let photos = [];
        if (req.file) {
            photos.push(req.file.path); // Agrega la ruta de la foto al array de fotos
        }

        // Crea una nueva instancia de la cabaña con los datos del formulario
        const newLodge = new Lodges({
            name,
            description,
            capacity,
            services,
            photos // Agrega la(s) foto(s) al objeto de datos de la cabaña
        });

        // Guarda la nueva cabaña en la base de datos
        const savedLodge = await newLodge.save();

        // Envía la respuesta con la cabaña recién creada
