...
export const login = async (req, res, next) => {
    try{
        ...
        const token = jwt.sign({id:user._id, email: user.email, first_name: user.first_name, last_name: user.last_name, isAdmin:user.isAdmin, isEmployee:user.isEmployee}, process.env.JWT_KEY, {}, (err,token) =>{
            if(err) throw err; 
            res.cookie("token", token,).status(200).json(user); 
        });

        const { password, isAdmin,isEmployee, ...otherDetails } = user._doc;
