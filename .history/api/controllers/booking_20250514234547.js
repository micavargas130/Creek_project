...
import { sendBookingEmail } from './path/al/archivoDeCorreo'; // Reemplazá con la ruta real
import User from '../models/User.js'; // Asumiendo que usás un modelo User
...
export const setStatusActive = async (req, res, next) => {
  try {
    ...
    const updatedBooking = await Booking.findByIdAndUpdate(
      ...
      .populate('status')
      .populate('user')  // Necesario para obtener el email y nombre
      .populate('lodge'); // Para obtener el nombre de la cabaña
    ...
    const userName = updatedBooking.user.username || updatedBooking.user.name;
    const bookingDetails = {
      checkIn: updatedBooking.lodge?.checkIn || 'N/A',
      checkOut: updatedBooking.lodge?.checkOut || 'N/A',
      lodge: {
    ...
    // Enviar correo
    await sendBookingEmail(userEmail, userName, null, bookingDetails);
