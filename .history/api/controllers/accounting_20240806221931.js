...
export const createAccounting = async (req, res, next) => {
    try {
        ...
        const paymentStatus = await PaymentStatus.findOne({status:"completada"});
        ...
        // Crear el estado inicial como 'pagada'
        const newStatus = new PaymentStatus({
            status: 'pagada', // Estado inicial 'pagada'
        });

        const savedStatus = await newStatus.save();

        // Asociar el estado a la entrada de Accounting
        savedAccounting.status.push(savedStatus._id);
        await savedAccounting.save();
