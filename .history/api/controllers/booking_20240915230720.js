...
import Prices from '../models/Lodges.js';
...
export const createBooking = async (req, res, next) => {
  try {
    ...
    const lodge = await Lodge.findById(req.body.lodge);
    if (!lodge) {
      return res.status(404).json({ error: 'El lodge no existe en la base de datos' });
    }
