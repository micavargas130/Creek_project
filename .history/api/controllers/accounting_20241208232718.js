...
export const getAccounting = async (req, res, next) => {
    try {
      const accounting = await Accounting.findById(req.params.id).populate('status').populate('lodge', 'lodge').populate('lodge',name').populate('tent').populate('user', ['first_name','last_name']);
      res.status(200).json(accounting);;
      res.status(200).json(accounting);
    } catch (err) {
      next(err);
