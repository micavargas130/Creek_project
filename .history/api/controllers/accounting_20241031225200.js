...
export const getAccountings = async (req, res, next) => {
    try {
        const accounting = await Accounting.find().populate('status').populate('user', 'name').populate('lodge', 'name') // Reemplaza 'name' con el campo que contiene el nombre de la cabaña
        .populate('tent', 'name');;
