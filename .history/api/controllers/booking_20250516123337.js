...
export const setStatusActive = async (req, res, next) => {
  try {
    ...
    const io = req.app.get("io"); // 👈 acceder a io desde Express
    io.emit("reserva-activada", updatedBooking); // 👈 enviar evento al frontend

    // Extraer datos necesarios
    const userEmail = updatedBooking.user.email;
    const userName = updatedBooking.user.username || updatedBooking.user.name;
    const bookingDetails = {
      checkIn: updatedBooking.checkIn || 'N/A',
      checkOut: updatedBooking.checkOut || 'N/A',
      lodge: {
