...
export const createLodge = async (req, res, next) => {
  try {
    console.log("📤 Recibiendo datos para crear lodge:", req.body);  // Ver qué datos están llegando

    const newLodge = new Lodge(req.body);
    const savedLodge = await newLodge.save();

    console.log("✅ Lodge creado:", savedLodge);
    res.status(200).json(savedLodge);
  } catch (error) {
    console.error("❌ Error al crear lodge:", error); // Ver el error exacto
...
export const getOccupiedDates = async (req, res) => {
  try {
    // Obtener las reservas de la cabaña
    const bookings = await Booking.find({
      lodge: req.params.id, // Buscar por el _id de la cabaña
    }).select('checkIn checkOut');

    // Generar las fechas ocupadas
    const occupiedDates = [];
    ...
    bookings.forEach((booking) => {
      ...
      // Agregar todos los días ocupados
      while (currentDate <= endDate) {
        occupiedDates.push(new Date(currentDate).toISOString().split('T')[0]); // Solo la fecha sin la hora
        currentDate.setDate(currentDate.getDate() + 1); // Incrementar al siguiente día
      }
    });

    // Devolver las fechas ocupadas como respuesta
    res.json({ occupiedDates });
  } catch (error) {
    console.error('Error al obtener las fechas ocupadas:', error);
    res.status(500).send('Error al obtener las fechas ocupadas');
  }
};
