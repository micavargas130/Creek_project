...
export const getTent = async(req, res, next) =>{
    try{
        const tent = await Tents.findByIdAndUpdate(req.params.id).populate('state')  //busca la carpa con el id que le pasamos 
...
export const getTents = async(req, res, next) =>{
    try{
        const tents = await Tents.find().populate('price')
