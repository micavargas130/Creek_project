...
export const getOccupiedDates = async (req, res) => {
  try {
    const { lodgeId } = req.params._id; // ID de la cabaña
    const lodge = await Lodges.findById(lodgeId)
      .populate('occupiedBy')
      .populate('state');
    ...
    // Fechas bloqueadas del campo unavailableDates
    let occupiedDates = lodge.unavailableDates || [];

    // Agregar fechas de reservas actuales
    if (lodge.occupiedBy) {
      const { checkIn, checkOut } = lodge.occupiedBy;
      let currentDate = new Date(checkIn);
      const endDate = new Date(checkOut);

      // Generar rango de fechas
      while (currentDate <= endDate) {
        occupiedDates.push(new Date(currentDate).toISOString());
