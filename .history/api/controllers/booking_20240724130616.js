...
export const getBooking = async (req, res, next) => {
  try {
    const booking = await Booking.findById(req.params.id).populate('user', 'first_name last_name').populate('status').populate('place');
...
export const setStatusCompleted = async (req, res, next) => {
  try {
    ...
    const updatedBooking = await Booking.findByIdAndUpdate(
      bookingId,
      { status: completedStatus.d }, // Referenciar el estado "Completada"
      { new: true }
