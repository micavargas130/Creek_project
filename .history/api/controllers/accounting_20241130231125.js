...
export const PaymentHistory = async (req, res, next) => {
    try {
        ...
        // Actualiza `remainingAmount` en `Accounting`
        accounting.remainingAmount = newRemainingAmount;
        accounting.status = await PaymentStatus.findOne({ status: newStatus });
        await accounting.save();
    
        // Crea un nuevo registro en `PaymentHistory`
 ...
 export const completePayment = async (req, res, next) => {
    try {
      ...
      // Busca el registro de Accounting por ID
      const accounting = await Accounting.findById(id);
      ...
      // Actualiza el estado y el monto restante
      accounting.remainingAmount = 0;
      accounting.status = await PaymentStatus.findOne({ status });
      await accounting.save();

      console.log("Accounting ID:", accounting._id);
console.log("Amount:", amount);
console.log("Status ID:", accounting.status ? accounting.status._id : "Status no definido");
  
      // Crear nueva entrada en PaymentHistory
      console.log("Creando historial de pago...");
const paymentHistory = new PaymentHistory({
  accounting: accounting._id,
  amount,
  status: accounting.status._id
});
console.log("Datos a guardar:", paymentHistory);
await paymentHistory.save();
console.log("Historial guardado.");
  
      res.status(200).json({ message: "Pago completado exitosamente", accounting, paymentHistory });
    } catch (error) {
