...
export const createLodge = async (req, res, next) => {
  const { name, description, capacity, services, file } = req.body;
  const base64Data = file.replace(/^data:image\/\w+;base64,/, ""); // Eliminar el encabezado
  const buffer = Buffer.from(base64Data, "base64");

  try {
    // Guardar el archivo en el servidor (ejemplo)
    fs.writeFileSync("ruta/de/destino/nombre-del-archivo.png", buffer);
    ...
    const newLodge = new Lodges({
      ...
      capacity,
      services,
      file: "ruta/de/destino/nombre-del-archivo.png" // Guardar la ruta del archivo en la base de datos
    });
