...
export const createSchedule = async (req, res) => {
  try {
    const { employee, task, date, startTime, endTime, status } = req.body;
    const newSchedule = new Schedule({ employee, task, date, startTime, endTime, status });
...
export const getAllSchedules = async (req, res) => {
  try {
    const schedules = await Schedule.find().populate("employee").populate("status");
...
export const getScheduleByEmployee = async (req, res) => {
  try {
    ...
    const schedules = await Schedule.find({ employee: employeeId }).populate("status");
    res.status(200).json(schedules);
  } catch (error) {
