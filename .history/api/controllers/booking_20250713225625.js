...
export const createBooking = async (req, res, next) => {
  try {
    ...
    const nights = localDates && localDates[0]?.startDate && localDates[0]?.endDate ? Math.max(1, Math.ceil(
    ...
    const overlappingBooking = await Booking.findOne({
      lodge: lodge,
      status: { $ne: "668ddd316630f103dda28cdf" }, 
      $or: [
        {
          checkIn: { $lt: checkOut },
          checkOut: { $gt: checkIn }
    ...
    console.log("over", overlappingBooking)
