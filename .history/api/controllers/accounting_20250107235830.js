...
export const getAccountings = async (req, res, next) => {
    try {
        const accounting = await Accounting.find().populate('status').populate('user', ['first_name','last_name']).populate({
...
export const getAccounting = async (req, res, next) => {
    try {
      ...
      }).populate('tent').populate('user', ['first_name','last_name']);
      res.status(200).json(accounting);;
      res.status(200).json(accounting);
    } catch (err) {
      next(err);
  ...
  export const getAccountingByFilter = async (req, res, next) => {
    try {
      const { filter } = req.query; // Obtén el filtro de la consulta
      let dateRange = {};
      const now = new Date();
  
      if (filter === "day") {
        dateRange = {
          $gte: new Date(now.setHours(0, 0, 0, 0)), // Inicio del día
          $lte: new Date(now.setHours(23, 59, 59, 999)), // Fin del día
        };
      } else if (filter === "month") {
        dateRange = {
          $gte: new Date(now.getFullYear(), now.getMonth(), 1), // Inicio del mes
          $lte: new Date(now.getFullYear(), now.getMonth() + 1, 0, 23, 59, 59), // Fin del mes
        };
      } else if (filter === "year") {
        dateRange = {
          $gte: new Date(now.getFullYear(), 0, 1), // Inicio del año
          $lte: new Date(now.getFullYear(), 11, 31, 23, 59, 59), // Fin del año
        };
      }
  
      // Encuentra registros en el rango de fechas
      const accounting = await Accounting.find({
        createdAt: dateRange, // Filtra por rango de fechas (asumiendo que tienes un campo `createdAt`)
      })
        .populate('status')
        .populate({
          path: 'lodge',
...
export const getPaymentHistoryByLodge = async (req, res, next) => {
    try {
      ...
      // Busca el historial de pagos en PaymentHistory relacionado con el mismo lodgeId
      const paymentHistory = await PaymentHistory.find({ accounting: { $in: payments.map(p => p._id) } }).sort({ date: 1 });
  
      // Combina los resultados de Accounting y PaymentHistory
      const combinedPayments = [...payments, ...paymentHistory];
  
      // Devuelve los resultados combinados o un mensaje indicando que no hay pagos
 ...
 export const paymentHistory = async (req, res, next) => {
    try {
      ...
      accounting.amount = accounting.totalAmount;
