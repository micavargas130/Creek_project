...
export const updateEmplot = async(req, res, next) =>{
    try{
        const updateEmplot = await Lodges.findByIdAndUpdate(req.params.id, {$set:req.body}, {new:true})  
        res.status(200).json(updateEmplot)
    }catch(err) {
        next(err);
    }
}
