...
export const getLastPrice = async (req, res, next) => {
  try {
    ...
    if (!lastPrice) {
      return res.status(404).json({ message: `No hay precios registrados para ${category}` });
    }
...
export const getPriceHistory = async (req, res, next) => {
  try {
    const { category } = req.params;
    const history = await PriceHistory.find({ category }).sort({ changedAt: -1 });
    
    if (!history) {
      return res.status(404).json({ message: `No hay historial de precios para ${category}` });
    }
