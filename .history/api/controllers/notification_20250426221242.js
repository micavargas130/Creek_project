...
import Tents from "../models/Tent.js";
import Notifications from "../models/Notification.js";
import cron from "node-cron";
...
export const deleteOldNotifications = async () => {
  ...
  export const checkTodayCheckouts = async () => {
    try {
      ...
      const lodgeBookings = await Bookings.find({
        "lodge.checkOut": { $gte: today, $lt: tomorrow }
      ...
      for (const booking of lodgeBookings) {
        ...
        const notification = new Notifications({
          type: "Checkout",
          message: `Hoy debe hacer check-out ${fullName} de la cabaña ${lodgeName}`,
          date: new Date(),
        });
      ...
      for (const tent of tentBookings) {
        ...
        const notification = new Notifications({
          type: "Checkout",
          message: `Hoy debe hacer check-out ${fullName} de la carpa en fila ${tent.location.row}, columna ${tent.location.col}`,
          date: new Date(),
        });
  ...
  // Ejecutarlo todos los días a las 6:00 AM
  cron.schedule("0 6 * * *", async () => {
    console.log("🔔 Revisando checkouts programados para hoy...");
    await checkTodayCheckouts();
    console.log("🔔 Proceso de checkouts finalizado.");
  });
