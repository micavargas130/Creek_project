...
export const createBooking = async (req, res, next) => {
  try {
    ...
    if (!prices) {
      return res.status(404).json({ error: 'Precios no encontrados' });
    }

    // Desestructurar el body y asegurarse de que los valores sean numéricos
    let { numberOfAdults, numberOfChildren } = req.body;
    numberOfAdults = Number(numberOfAdults);
    numberOfChildren = Number(numberOfChildren);

    // Validar que los valores sean números
    if (isNaN(numberOfAdults) || isNaN(numberOfChildren)) {
      return res.status(400).json({ error: 'Número de adultos o niños inválido' });
    }

    // Calcular el totalAmount usando los precios
    const totalAmount = (numberOfAdults * prices.priceAdult) + (numberOfChildren * prices.priceChild);
    
    if (isNaN(totalAmount)) {
      return res.status(400).json({ error: 'El cálculo del monto total es inválido' });
    }

    // Crear la nueva reserva (Booking)
