  ...
  try {
    ...
    const mailOptions = {
      ...
      html: `
        ...
        <p>Para confirmar la misma, necesitamos que envíes una seña que corresponde al 30% del monto de la reserva (esto se deducira del monto a abonar una vez ll</p>
        <p>Detalles de la cuenta</p>
        <ul>
          <li><strong>Monto a transferir (30% del monto de la reserva): </strong> ${(bookingDetails.totalPrice)*0.3}</li>
          <li><strong>CBU:</strong> ${userEmail}</li>
          <li><strong>Titular de la cuenta:</strong> ${userEmail}</li>
          <li><strong>Banco:</strong> ${userEmail}</li>
        </ul>
         <p>Una vez transferido por favor enviar comprobante al siguiente whatsapp:</p>
...
export const sendCancellationEmail = async (userEmail, userName, bookingDetails) => {
  try {
    const mailOptions = {
      from: "Camping Arroyito <arroyitocamping@gmail.com>",
      to: userEmail,
      subject: "Registro de tu reserva - Camping Arroyito",
      html: `
        <h2>Hola, ${userName}.</h2>
        ...
        <ul>
          <li><strong>Fecha de ingreso:</strong> ${bookingDetails.checkIn}</li>
          <li><strong>Fecha de salida:</strong> ${bookingDetails.checkOut}</li>
          <li><strong>Cabaña/Carpa:</strong> ${bookingDetails.lodge.name}</li>
          <li><strong>Precio Total:</strong> $${bookingDetails.totalPrice}</li>
        </ul>
        <p>Para confirmar tu recerva, requerimos una seña del 30% del total de la rese</p>
...
export const sendBookingRemainderEmail = async (userEmail, userName, bookingDetails) => {
  try {
    const mailOptions = {
      from: "Camping Arroyito <arroyitocamping@gmail.com>",
      to: userEmail,
      subject: "Confirmacion de tu reserva - Camping Arroyito",
      html: `
        <h2>Hola, ${userName}.</h2>
