...
test('PUT /lodges/:id debe actualizar una cabaña', async () => {
  ...
  test('PUT /lodges/set-occupied/:id debe cambiar estado a ocupado', async () => {
    ...
    const response = await request(app).post(`/lodges_x_status/`).send({newLodge});
    console.log("cuerpo", response.status)
    assert.strictEqual(response.status, 200);
     //assert.strictEqual(response.body, "682be492427c566d515638c9");
  });
