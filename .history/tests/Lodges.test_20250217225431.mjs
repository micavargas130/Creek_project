...
after(async (done) => {  // Añadimos `done` para asegurar el cierre correcto
  ...
  // Esperamos un poco antes de finalizar el proceso
  setTimeout(() => {
    done();  // Llamamos a `done` para asegurar que el test termine
