...
test('PUT /bookings/:id debe actualizar una reserva', async () => {
  ...
  const response = await request(app).put(`/bookings/${createdBookingId}/updateStatusCompleted`).send(updateData);
  assert.strictEqual(response.status, 200);
  assert.strictEqual(response.body.status, 'Completada');
});
