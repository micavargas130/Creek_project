...
test('POST /accounting/pay/:id debe registrar un pago parcial', async () => {
  const accounting = {
    amount: 500,
    totalAmount: 1000,
    remainingAmount: 500,
  ...
  const createRes = await request(app).post('/accounting/createAccounting').send(accounting);
  const id = createRes.body._id;
  createdAccountingId = id;


  const response = await request(app).post(`/accounting/pay/${id}`).send({ additionalAmount: 250 });

  console.log(response.status);
  assert.strictEqual(response.status, 200);
  assert.ok(response.body.accounting.remainingAmount === 250);
});
